...
public class Grid : MonoBehaviour
{
    ...
    public LayerMask ObstacleMask;//This is the mask that the program will look for when trying to find obstructions to the path.
    ...
    {
        fNodeDiameter = NodeRadius * 2;//Double the radius to get diameter
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            ...
            {
                Vector3 worldPoint = bottomLeft + Vector3.right * (x * fNodeDiameter + NodeRadius) + Vector3.forward * (y * fNodeDiameter + NodeRadius);//Get the world co ordinates of the bottom left of the graph
    ...
    private void TestOnDrawGizmos()
    {
        ...
        if (NodeArray != null)//If the grid is not empty
        {
            foreach (Node n in NodeArray)//Loop through every node in the grid
            {
                ...
                Gizmos.DrawCube(n.position, Vector3.one * (fNodeDiameter - DistanceBetweenNodes));//Draw the node at the position of the node.
            }
        }
    }
}