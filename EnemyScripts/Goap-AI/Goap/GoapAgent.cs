...
public sealed class GoapAgent : MonoBehaviour
{
	...
	private void createIdleState()
	{
		idleState = (fsm, gameObj) => {
			...
			// where things are in the world/what's going on
			// chasingPlayer -> true
    ...
    //        float distanceApart = Vector3.Distance(gameObj.transform.position, action.target.transform.position);

    //        if (distanceApart <= 5)
    //        {
    //            Debug.Log("150: within 5 range");
    ...
    //        else
    //        {
    //            //Debug.Log($"gamnObj: {gameObj.name} : {gameObj.transform.position}");
    //            //Debug.Log($"action.target: {action.target.name} : {action.target.transform.position}");
    //            Debug.Log($"159: not in range {action.actionNameName}");
				////action.
    //        }
	...
	private void createPerformActionState()
	{
		...
		performActionState = (fsm, gameObj) => {
			...
			if (!hasActionPlan())
			{
				...
				fsm.popState();
				// go back to idle - create new plan
				fsm.pushState(idleState);
			...
			if (hasActionPlan())
			{
				...
				if (inRange)
				{
					...
					if (!success)
					{
						//Debug.Log("215: In range - ACTION FAILED");
				...
				{
					//Debug.Log("225: Out of range... moving");
